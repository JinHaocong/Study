...
它会以**一个**或**多个**文件作为打包的入口，将我们整个项目所有文件编译组合成一个或多个文件输出出去。
...
**npx指令会将node_modules中的.bin目录添加为临时环境变量，就可以访问其中的一些应用程序**
...
- `css-loader`：**负责将 Css 文件编译成 Webpack 能识别的模块(common js)**
...
原因是 Webpack 对 js 处理是**有限的**，**只能编译 js 中 ES 模块化语**法，不能编译其他语法，导致 js 不能在 IE 等浏览器运行，所以我们希望做一些兼容性处理。
...
主要用于将 ES6 语法编写的代码转换为**向后兼容**的 JavaScript 语法，以便能够运行在当前和旧版本的浏览器或其他环境中
