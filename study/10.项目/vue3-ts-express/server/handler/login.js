...
exports.login = async (req, res) => {
  try {
    ...
    const [queryData] = await db.query(querySQL, account) || [];
    if (!queryData.length) {
      return res.error('登录失败：用户不存在');
    }

    // step 2：对前端传过来的密码进行解密
    const compareResult = bcrypt.compareSync(password, queryData[0].password);
    if (!compareResult) {
      return res.error('登录失败：密码错误');
    }

    // step 3：对账号是否冻结做判定
    if (queryData[0].status === '1') {
      return res.error('登录失败：账号被冻结');
    }
