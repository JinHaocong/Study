...
!(function (a) {
  ...
  function h(b, c) {
    ...
    const f = a(b).data();
    const g = {};
    const h = new RegExp(`^${c.toLowerCase()}([A-Z])`);
    c = new RegExp(`^${c.toLowerCase()}`);
    for (const i in f) c.test(i) && ((e = i.replace(h, d)), (g[e] = f[i]));
    return g;
  }
  function i(b) {
    const c = {};
  ...
  var j = (function () {
    const b = {
      ...
      contains(a) {
        for (let b = a && a.valueOf(), c = 0, d = this.length; c < d; c++)
          if (this[c].valueOf() - b >= 0 && this[c].valueOf() - b < 864e5)
            return c;
        return -1;
      },
      remove(a) {
  ...
  k.prototype = {
    ...
    _process_options(b) {
      ...
      const e = (this.o = a.extend({}, this._o));
      let f = e.language;
      ...
      const g = r.parseFormat(e.format);
      e.startDate !== -1 / 0 &&
        (e.startDate
      ...
      let h = String(e.orientation).toLowerCase().split(/\s+/g);
      let i = e.orientation.toLowerCase();
      if (
      ...
      if (
        ...
        const j = e.defaultViewDate.year || new Date().getFullYear();
        const k = e.defaultViewDate.month || 0;
        const l = e.defaultViewDate.day || 1;
    ...
    _trigger(b, c) {
      const d = c || this.dates.get(-1);
      const e = this._utc_to_local(d);
    ...
    _utc_to_local(a) {
      if (!a) return a;
      let b = new Date(a.getTime() + 6e4 * a.getTimezoneOffset());
      return (
    ...
    setDates() {
      const b = a.isArray(arguments[0]) ? arguments[0] : arguments;
      return (
    ...
    place() {
      ...
      const b = this.picker.outerWidth();
      const c = this.picker.outerHeight();
      const d = a(this.o.container);
      const e = d.width();
      const f =
        this.o.container === 'body' ? a(document).scrollTop() : d.scrollTop();
      const g = d.offset();
      const h = [0];
      this.element.parents().each(function () {
        const b = a(this).css('z-index');
        b !== 'auto' && Number(b) !== 0 && h.push(Number(b));
      });
      const i = Math.max.apply(Math, h) + this.o.zIndexOffset;
      const j = this.component
          ...
          : j.left < 0
            ...
            : m + b > e
              ...
              : this.o.rtl
                ...
                : this.picker.addClass('datepicker-orient-left');
      let o;
      let p = this.o.orientation.y;
      if (
        (p === 'auto' && ((o = -f + n - c), (p = o < 0 ? 'bottom' : 'top')),
    ...
    getClassNames(b) {
      const c = [];
      const f = this.viewDate.getUTCFullYear();
      const g = this.viewDate.getUTCMonth();
      const h = d();
      return (
        b.getUTCFullYear() < f ||
    ...
    fill() {
      let e;
      let f;
      const g = new Date(this.viewDate);
      const h = g.getUTCFullYear();
      const i = g.getUTCMonth();
      const j =
      ...
      const m = this.o.endDate !== 1 / 0 ? this.o.endDate.getUTCMonth() : 1 / 0;
      const n = q[this.o.language].today || q.en.today || '';
      const o = q[this.o.language].clear || q.en.clear || '';
      const p = q[this.o.language].titleFormat || q.en.titleFormat;
      const s = d();
      const t =
        (!0 === this.o.todayBtn || this.o.todayBtn === 'linked') &&
        s >= this.o.startDate &&
        s <= this.o.endDate &&
      ...
      if (!isNaN(h) && !isNaN(i)) {
        ...
        for (var x, y, z = []; u.valueOf() < w; ) {
          if (
            ...
            const A = new Date(+u + ((this.o.weekStart - x - 7) % 7) * 864e5);
            const B = new Date(Number(A) + ((11 - A.getUTCDay()) % 7) * 864e5);
            var C = new Date(
              Number((C = c(B.getUTCFullYear(), 0, 1))) +
                ((11 - C.getUTCDay()) % 7) * 864e5
            );
            const D = (B - C) / 864e5 / 7 + 1;
          ...
          this.o.beforeShowDay !== a.noop &&
            ...
            z.push(
              `<td class="${y.join(' ')}"${
                e ? ` title="${e}"` : ''
              } data-date="${u.getTime().toString()}">${E}</td>`
            ),
            (e = null),
        ...
        if (
          ...
          const H = this;
          a.each(G, (c, d) => {
            const e = new Date(h, c, 1);
            let f = H.o.beforeShowMonth(e);
            f === b
              ? (f = {})
              : typeof f === 'boolean'
                ? (f = { enabled: f })
    ...
    updateNavArrows() {
      if (this._allow_update) {
        let a;
        let b;
        const c = new Date(this.viewDate);
        const d = c.getUTCFullYear();
        const e = c.getUTCMonth();
        const f =
          this.o.startDate !== -1 / 0
            ? this.o.startDate.getUTCFullYear()
            : -1 / 0;
    ...
    moveMonth(a, b) {
      ...
      let c;
      let d;
      let e = new Date(a.valueOf());
      let f = e.getUTCDate();
      const h = e.getUTCMonth();
      const i = Math.abs(b);
      if (((b = b > 0 ? 1 : -1), i === 1))
        (d =
          b === -1
        ...
        for (let j = 0; j < i; j++) e = this.moveMonth(e, b);
        (c = e.getUTCMonth()),
          e.setUTCDate(f),
          (d = function () {
    ...
    keydown(a) {
      ...
      let c;
      let d = !1;
      let e = this.focusDate || this.viewDate;
      switch (a.keyCode) {
        case 27:
          this.focusDate
            ? ((this.focusDate = null),
  ...
  l.prototype = {
    ...
    updateRanges() {
      const b = a.map(this.dates, (a) => a.valueOf());
    ...
    dateUpdated(c) {
      if (!this.updating) {
        this.updating = !0;
        const d = a.data(c.target, 'datepicker');
        if (d !== b) {
          const e = d.getUTCDate();
          const f = this.keepEmptyValues;
          const g = a.inArray(c.target, this.inputs);
          let h = g - 1;
          let i = g + 1;
          const j = this.inputs.length;
          if (g !== -1) {
            if (
              (a.each(this.pickers, (a, b) => {
  ...
  const m = a.fn.datepicker;
  var n = function (c) {
    const d = Array.apply(null, arguments);
    d.shift();
    let e;
    if (
      (this.each(function () {
        const b = a(this);
        let f = b.data('datepicker');
        const g = typeof c === 'object' && c;
        if (!f) {
          const j = h(this, 'date');
          const m = a.extend({}, o, j, g);
          const n = i(m.language);
          const p = a.extend({}, o, n, j, g);
  ...
  var p = (a.fn.datepicker.locale_opts = ['format', 'rtl', 'weekStart']);
  ...
  var r = {
    ...
    nonpunctuation: /[^ -\/:-@\u5e74\u6708\u65e5\[-`{-~\t\n\r]+/g,
    parseFormat(a) {
      if (typeof a.toValue === 'function' && typeof a.toDisplay === 'function')
        return a;
      const b = a.replace(this.validParts, '\0').split('\0');
      const c = a.match(this.validParts);
      if (!b || !b.length || !c || c.length === 0)
    ...
    parseDate(c, e, f, g) {
      ...
      function i() {
        const a = this.slice(0, j[n].length);
        const b = j[n].slice(0, a.length);
        return a.toLowerCase() === b.toLowerCase();
      }
      if (!c) return b;
      if (c instanceof Date) return c;
      if ((typeof e === 'string' && (e = r.parseFormat(e)), e.toValue))
      ...
      const s = { yesterday: '-1d', today: '+0d', tomorrow: '+1d' };
      if (
        (c in s && (c = s[c]),
      ...
      let u;
      const v = {};
      const w = ['yyyy', 'yy', 'M', 'MM', 'm', 'mm', 'd', 'dd'];
      const x = {
        yyyy(a, b) {
    ...
    formatDate(b, c, d) {
      if (!b) return '';
      if ((typeof c === 'string' && (c = r.parseFormat(c)), c.toDisplay))
      ...
      (e.dd = (e.d < 10 ? '0' : '') + e.d),
        ...
        (b = []);
      for (
        let f = a.extend([], c.separators), g = 0, h = c.parts.length;
        g <= h;
        g++
      )
